...
- [diffense — kb-first PR review experience](design-diffense.md) —
  *proposed 2026-05-28*. The review surface for brr-generated PRs,
  built around the half-of-a-brr-PR-is-kb pain. Inspect-mode model:
  reviews become a navigable graph of cards (item / walkthrough /
  uncertainty kinds) with two-axis lore (what-it-is + what-it-enables),
  load-bearing per-kind stat blocks, and tests-grounded usage demos.
  Two-layer architecture — a JSON **review pack** generated by the
  runner at publish time, rendered by multiple targets over one
  proposed **Textual** substrate (TUI + `textual serve` web, plus a
  forge PR-body projection as the v0 surface and a future brnrd hosted
  view). Six discipline clamps (sharp / helpful / honest /
  non-prescriptive / emit-iff-honest / substrate-honest) keep it
  Occam's-razor sharp; agent **uncertainty cards** are first-class and
  read first. Open: pack schema, substrate spike, aesthetic lock,
  project boundary.
