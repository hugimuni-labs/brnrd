...
| --- | --- | --- | --- | --- |
| Codex CLI | `codex-cli 0.141.0` | `codex exec --model`, `-c model=...`, `--profile`, `--json`, `exec resume`, app-server commands | **`token_count` events (evt-9yvh):** `codex exec --json` emits `event_msg` entries with `payload.type=="token_count"` (cumulative input / cached / output / reasoning tokens, since codex commit 0269096); same events land in `$CODEX_HOME/sessions/*.jsonl` (what `ccusage` reads). So spend is **derivable** (tokens × a price table) and context headroom is computable — but neither is *handed over* like Claude. Declarative TUI status line (`[tui] status_line=[…]`) shows context/tokens/limits but has **no external-command seam** to intercept. | Subscription quota (5h/weekly) is **not** exposed to a headless `codex exec` — TUI/`/status` only; programmatic exposure is requested-not-shipped (OpenAI issues #15281, #19555, #17827). Spend requires brr to own a price table (no `cost_usd` gauge). brr drives `codex exec` *without* `--json` today, so capturing `token_count` is a real output-parsing change, not a tweak. |
| Claude Code | `2.1.191` | `--model`, `--fallback-model`, named/resumable sessions, `--output-format stream-json`, `--max-turns` | **`statusLine` JSON (subscription, evt-e1gl):** the configured status-line command receives session JSON on stdin carrying `rate_limits.five_hour/seven_day.used_percentage` + `.resets_at`, `context_window.remaining_percentage`, and `cost.total_cost_usd`; `/usage` shows the same interactively. This makes subscription quota/cost/context **level-readable without API-key auth** — a hook-shaped collector. Also: `--max-budget-usd` (print mode), Anthropic API rate-limit headers, Admin API usage/cost reports. | statusLine schema is newer Claude Code surface — smoke-verify firing/fields before relying (pitfall: fire it before you rule on it). Codex's analog is `token_count` events, not a statusLine command — a derived, heavier collector (see Codex row + `design-resident-boundary.md` §8c). |
...
| Readable local gauge | **Claude Code `statusLine` JSON** (subscription `rate_limits.*` used% + resets, `context_window.remaining_percentage`, `cost.total_cost_usd`) | statusLine command brr registers alongside hooks; captured into the portal collector. Best-effort (estimated cost, consumed% not remaining) but a real gauge, not just error text. evt-e1gl |
| Derived local (Codex) | `token_count` events (`codex exec --json` or `$CODEX_HOME/sessions/*.jsonl`) → spend via price table, context via token counts; quota stays error-text/suggestion edge-only | a second collector shape, not a statusLine port (evt-9yvh, `design-resident-boundary.md` §8c) |
| Best effort local | Codex subscription error text; CLI suggestions; manually supplied `.brr/runner-quota.json` | snapshot or failure classifier, source marked best-effort |
