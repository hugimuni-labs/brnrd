# Subject: runs and branching

Hub page for how brr turns incoming events into runs, isolates the
...
Runs are mechanical. An event from a gate becomes `Run.from_event`
with no LLM triage step. The run carries the lead event body, source,
conversation key, resolved environment, status, and runtime metadata,
and persists as `.brr/runs/<run-id>/run.md`.
...
classify "small" versus "large" requests ahead of time.

For worktree-backed runs, brr resolves a deterministic
`PublishPlan`: seed ref, optional `target_branch` (when the
...
remote-tracking ref at run start for force-with-lease pushes. It then
creates `.brr/worktrees/<run-id>/` on a fresh `brr/<run-id>` branch
from the seed ref. The agent owns the runtime branching choice:

- commit on the original `brr/<run-id>` branch when the plan is
  right;
- switch to a new or existing named branch when the run instruction — or
...
`brr/<run-id>` branch instead, records a branch-setup notice on the
run, and prints the fallback. Because the run branch was sprouted
...
to publish. Finalize never touches a non-run ref. Worktree teardown
...
step only for the opt-in diffense review-pack path: when both diffense
flags are enabled, the resident projects the pack and sends `gate:
forge`, which the GitHub gate turns into an idempotent PR create/update.
Generic PR-first handoff remains future portal work, not part of the
publish kernel and not a new user-facing `brr` subcommand.
...
`target_branch`, `base_branch`, then `branch`) name the `target_branch`
and the seed. When a remote-tracking ref
...
records no expected publish target; the committed run branch is
...
- `ready` — the run branch has commits; `publish_branch` names what
  to ship (the original `brr/<run-id>` if the agent stayed put, or
  the agent's chosen branch otherwise).
- `nothing` — agent stayed on the run branch and made no commits;
  run branch is deleted along with the worktree.
...
reading `run.meta["publish_branch"]`, `run.meta["target_branch"]`, and
`run.meta["expected_remote_oid"]`:
...
| refspec | agent kept `brr/<run-id>` but event named a different `target_branch` | `git push origin brr/<run-id>:<target>` |
...
routes through it. `sync.refresh_before_run` already fetches origin
and best-effort fast-forwards before the resolver runs, and the
resolver prefers `<remote>/<target>` when present, so a follow-up run
seeds from the previous run's published state even when the
...
2. [`decision-remove-triage.md`](decision-remove-triage.md) for why
   event-to-run construction is mechanical and branching moved to runtime.
