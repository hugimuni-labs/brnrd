...
`title`, but only when diffense PR creation is explicitly enabled.
`_deliver_out_of_bound` maps that to the GitHub gate; the GitHub delivery
closure opens or refreshes the PR idempotently and otherwise refuses the
send.
