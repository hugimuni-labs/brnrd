...
SLIDES = [{'label': 'TITLE',
  'headline': 'brnrd — an AI pal that lives on your machine',
  'body': ['Built by two people, and by the pal itself. Open source. Runs on the subscription you already '
           'pay for.',
 ...
 {'label': 'PROBLEM',
  ...
  'body': ["Agents got powerful. The way we work with them didn't.",
 ...
 {'label': 'THE WALL',
  ...
  'body': ['It budgets your quota, hands chores to cheaper models, and parks itself at the wall instead of '
           'dying halfway.',
           'Receipt, 5 Sep 2026: Codex hit its weekly wall at 18:16. The seat continued on Claude — same '
           'thread, same memory.']},
 {'label': "WHILE YOU'RE AWAY",
  'headline': 'So you can leave.',
  'body': ['Receipt, 13 Sep 2026: the founder slept. The seat woke itself at 02:00 and 07:00 and merged a '
           'fix each time. By morning its run page listed six merged PRs and one closed issue — one run, one '
           'thread, no terminal open.']},
 {'label': 'FROM A PHONE',
  'headline': 'Message it. It ships.',
  'body': ['Receipt, 1 Sep 2026: a change requested from WhatsApp; the PR opened 76 seconds after the '
           'follow-up; merged from the phone. No terminal was opened.']},
 {'label': 'BUILT WITH IT, BY IT',
  'headline': 'The repo is the demo.',
  'body': ["As of 13 Sep 2026: 2,946 commits on main since March — 1,898 of them the resident's own (64%). "
           '1,325 merged. 27 releases. A 272-page knowledge base it writes and keeps.',
           'It edited the video on slide 6, asked another agent to paint slide 8, and built this deck.',
           'It reviewed this deck and found it had no numbers. Twice.']},
 {'label': 'WE ARE SMALL',
  'headline': "We are small. It doesn't matter.",
  'body': ['Two founders. Zero raised. Three daily users — one of them is the pal. No marketing team; this '
           'deck is the marketing.',
           'What we lack: money, and anyone who has sold software before.',
           "We're building it anyway — an AI pal you play: a resident on your machine, a map of your project "
           'it walks, a game about the work.',
           'Watch, or join. brnrd.dev · open source · $7 flat hosted, live.',
           '(the pal decided what this should be and asked another agent to draw it)']},
 ...
 {'label': 'TEAM & WEB SUMMIT',
  ...
  'body': ['Alexandra Lapunova — co-founder & President',
           'Arseni Lapunov — co-founder & CTO',
           'HugiMuni SAS, France',
           'At Web Summit: show it, meet the first users and partners, and the people building agents who '
           'want a pal above the vendors.',
           'brnrd.dev']}]

MOVIES = {
    "stage": "loop-3-stage.mp4",
    "runpage": "loop-5-the-run-page.mp4",
}

PICTURES = {
    "lose": "slide2-lose.png",
    "interface": "slide3-interface.png",
    "wall": "slide4-wall.png",
    "receipt": "slide4-receipt.png",
}
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--pictures-dir", type=Path, required=True)
...
def build(prs: Presentation, v10: Presentation, movies: dict[str, Path], posters: dict[str, Path], painting: Path, pictures: dict[str, Path]):
    ...
    add_rich_text(slide, [("brnrd", True, False, INK), (" — ", False, False, COOL), ("an AI pal that lives on your machine", False, True, COOL)], 0.78, 1.47, 10.7, 1.15, size=33)
    ...
    add_picture(slide, pictures["lose"], 8.55, 0.72, 4.15, 5.4)
    ...
    add_picture(slide, pictures["interface"], 6.75, 0.85, 5.2, 4.1)
    ...
    add_picture(slide, pictures["wall"], 6.75, 0.85, 5.2, 2.275)
    add_picture(slide, pictures["receipt"], 6.75, 3.32, 5.2, 1.45)
    ...
    add_text(slide, "TWO WAKES", 0.75, 5.05, 2.2, 0.3, size=12.5, color=COOL, bold=True)
    add_text(slide, "SIX PRs · ONE ISSUE CLOSED", 3.28, 5.05, 3.4, 0.3, size=12.5, color=AMBER, bold=True)
    add_movie(slide, movies["runpage"], posters["runpage"], (7.3, 0.9, 5.4, 3.375))
    ...
    add_text(slide, SLIDES[8]["headline"], 0.68, 1.08, 8.35, 1.28, size=32)
    add_text(slide, SLIDES[8]["body"][0], 0.72, 2.75, 8.2, 1.8, size=19.0, color=MUTED, line_spacing=1.12)
    add_rect(slide, 0.75, 4.78, 8.1, 0.018, AMBER)
    add_text(slide, SLIDES[8]["body"][1], 0.72, 5.0, 8.2, 1.1, size=17.5, color=COOL, line_spacing=1.1)
    # 10 — team, preserving v10's three-column layout
    slide = prs.slides.add_slide(blank)
    add_background(slide, backgrounds["wide"], 0.84)
    add_chrome(slide, SLIDES[9]["label"], "10")
    add_text(slide, SLIDES[9]["headline"], 0.68, 1.08, 7.2, 0.72, size=33)
    columns = [
    ...
    add_text(slide, SLIDES[9]["body"][3], 0.8, 5.04, 9.9, 0.8, size=17.0, color=MUTED)
    add_text(slide, SLIDES[9]["body"][4], 0.8, 6.0, 2.0, 0.35, size=20.0, color=AMBER)
...
def spec_fragments(spec: Path) -> list[str]:
    ...
    start = next(i for i, line in enumerate(lines) if line.startswith("## v12 — the slides"))
    fragments: list[str] = []
    for line in lines[start + 1:]:
        ...
        if len(cells) < 5 or cells[0] in {"#", "---"} or not re.fullmatch(r"(?:[1-9]|10)", cells[0]):
            continue
    ...
    if not fragments:
        raise AssertionError("could not parse the v12 table from the spec")
    return fragments
...
def verify_media(output: Path, source_movies: Iterable[Path], painting: Path):
    ...
    if len(mp4_entries) != 2:
        raise AssertionError(f"expected two embedded MP4s, found {len(mp4_entries)}")
    ...
    print("MEDIA CHECK: PASS (2/2 MP4s embedded)")
...
def main():
    ...
    pictures = {key: args.pictures_dir / filename for key, filename in PICTURES.items()}
    for picture in pictures.values():
        if not picture.exists():
            raise SystemExit(f"required input does not exist: {picture}")
    ...
    with tempfile.TemporaryDirectory(prefix="brnrd-deck-v12-") as temp_name:
        temp = Path(temp_name)
        ...
        prs.core_properties.title = "brnrd — Web Summit 2026 pitch deck v12"
        ...
        build(prs, v10, movies, posters, darkened_painting, pictures)
