...
def test_register_then_list(tmp_path):
    brr = tmp_path / ".brr"
    entry = presence.register(brr, kind="daemon", stream="telegram:1:", task_id="t1")
    assert entry["id"]
...
def test_list_is_oldest_first(tmp_path):
    brr = tmp_path / ".brr"
    a = presence.register(brr, kind="daemon", task_id="a", now=100.0)
    b = presence.register(brr, kind="session", task_id="b", now=200.0)
...
def test_heartbeat_refreshes_and_keeps_alive(tmp_path):
    brr = tmp_path / ".brr"
    entry = presence.register(brr, kind="daemon", task_id="t1", now=100.0)
    ...
    presence.register(brr, kind="daemon", task_id="t1", entry_id=entry["id"], now=100.0)
...
def test_deregister_removes(tmp_path):
    brr = tmp_path / ".brr"
    entry = presence.register(brr, kind="session", task_id="t1")
...
def test_stale_entry_is_pruned_on_read(tmp_path):
    brr = tmp_path / ".brr"
    entry = presence.register(brr, kind="daemon", task_id="t1", now=1000.0)
...
def test_dead_pid_same_host_is_pruned(tmp_path):
    ...
    proc.wait()
    dead = proc.pid
    entry = presence.register(brr, kind="session", task_id="t1", pid=dead)
