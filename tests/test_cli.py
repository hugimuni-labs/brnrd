...
def test_portal_wrap_prints_state_when_token_changes(tmp_path, capsys, monkeypatch):
    import subprocess
    ...
    assert main(["portal", "wrap", "--path", str(state), "--", "mock", "arg"]) == 7

    err = capsys.readouterr().err
    assert "[brr portal update]" in err
    assert "token=after" in err
    assert "evt-2 telegram: new follow-up" in err


def test_portal_wrap_stays_quiet_when_token_is_unchanged(tmp_path, capsys, monkeypatch):
    import subprocess
        ...
        json.dumps(
            {
                "version": 1,
                "change_token": "same",
                "run": {"id": "run-1", "event_id": "evt-1"},
            }
        ),
        encoding="utf-8",
    )
    monkeypatch.setattr(subprocess, "call", lambda _command: 0)

    assert main(["portal", "wrap", "--path", str(state), "--", "mock"]) == 0

    assert capsys.readouterr().err == ""


def test_portal_wrap_always_prints_state(tmp_path, capsys, monkeypatch):
    import subprocess
    ...
    assert (
        main(["portal", "wrap", "--always", "--path", str(state), "--", "mock"])
        == 0
    )

    assert "[brr portal update]" in capsys.readouterr().err
