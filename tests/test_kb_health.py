...
def test_compute_graph_stats_records_run_touched_count(tmp_path):
    ...
    stats = kb_health.compute_graph_stats(
        tmp_path,
        run_touched=["kb/subject-hub.md", "kb/log.md", "AGENTS.md"],
    )

    assert stats.run_touched_count == 3


def test_compute_graph_stats_run_touched_defaults_to_zero(tmp_path):
    ...
    assert stats.run_touched_count == 0


def test_format_graph_stats_surfaces_run_touched_count(tmp_path):
    ...
    stats = kb_health.compute_graph_stats(
        tmp_path,
        run_touched=["kb/subject-hub.md", "kb/log.md"],
    )
    block = kb_health.format_graph_stats(stats)

    assert "run touched 2 kb / AGENTS.md pages this run" in block


def test_format_graph_stats_omits_touched_line_when_zero(tmp_path):
    """Skip-fast and zero-touch runs shouldn't see a stale
    'run touched 0 ...' bullet."""
    _write(tmp_path / "kb" / "index.md", "# Index\n")
    _write(tmp_path / "kb" / "subject-hub.md", "# Hub\n")
