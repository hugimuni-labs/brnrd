...
interim response partials before the terminal response and closes delivery
with status instead of deletion.
"""
...
def test_single_response_backward_compatible(tmp_path):
    ...
    assert protocol._read_event(ev["_path"])["status"] == "delivered"
    assert protocol.response_exists(responses, ev["id"])
...
def test_partials_stream_before_terminal_and_in_order(tmp_path):
    ...
    # now the thought finishes: terminal and event are retained as receipts
    ...
    assert protocol._read_event(ev["_path"])["status"] == "delivered"
    assert protocol.partials_dir(responses, ev["id"]).exists()
...
def test_done_with_no_terminal_still_closes_delivery(tmp_path):
    # A done event whose terminal response is missing (e.g. failure path
    # wrote none) must still be closed after its partials drain, so
