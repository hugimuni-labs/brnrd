...
const CASES = [
	{ name: 'both', body: { commit: 'da86b3c2f19a4b7c8e3d5061a2b9c4d7e8f01234', built_at: '2026-09-01T20:12:00+00:00', started_at: '2026-09-01T20:14:00+00:00' } },
	{ name: 'commit-only', body: { commit: 'da86b3c2f19a4b7c8e3d5061a2b9c4d7e8f01234', built_at: null, started_at: null } },
...
async function main() {
	...
	try {
		...
		for (const vp of [{ w: 390, h: 844, tag: '390' }, { w: 1440, h: 900, tag: '1440' }]) {
			for (const c of CASES) {
				...
				console.log(`[${vp.tag} · ${c.name}] header height=${box ? Math.round(box.height) : '?'} hOverflow=${overflow}`);
				console.log(`   header text: ${JSON.stringify(txt).slice(0, 300)}`);
				await page.screenshot({ path: `${OUT}/${vp.tag}-${c.name}.png`, clip: box ? { x: 0, y: Math.max(0, box.y - 8), width: vp.w, height: Math.min(vp.h, box.height + 40) } : undefined });
				await context.close();
			}
		}
		await browser.close();
