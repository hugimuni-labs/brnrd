...
// /v1/dashboard/telegram-pair/{code}` (`dashboard.py`), scoped to the
...
export async function fetchTelegramPairStatus(
	code: string,
	fetchImpl: typeof fetch = fetch
): Promise<TelegramPairStatus> {
	const res = await fetchImpl(`/v1/dashboard/telegram-pair/${encodeURIComponent(code)}`, {
		credentials: 'include'
	});
	if (res.status === 401) {
		throw new ReposAuthError('not signed in');
