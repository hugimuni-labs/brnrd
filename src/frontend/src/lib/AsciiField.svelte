<script lang="ts">
	// The ascii field — the camera over the unbounded room (#1652),
	// extracted from /ascii (2026-08-31, the water-line rework's rung 1:
	// "the ascii field inside… has the centering/actor-following
	// implemented already, so it fits"). /ascii remains the reference
	// debugger view; /daily embeds the same component as its scene.
	// One component, two routes — no renderer gets a semantic branch of
	// its own.
	//
	...
	interface Props {
		...
		legendDefault?: boolean;
		/** show the legend toggle at all */
		legendToggle?: boolean;
		/** replay the reference trace instead of polling the wires */
		demo?: boolean;
	}
	let {
		rows = 26,
