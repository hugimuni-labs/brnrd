<script lang="ts">
	...
	import { inlineTokens, markdownBlocks, type SurfaceResponse } from './surface';
	...
	let selected = $derived(
		data.files.find((file) => file.path === selectedPath) ??
			data.files.find((file) => file.path === 'index.md') ??
			data.files[0] ??
			null
	);
...
{#snippet inline(text: string)}
	{#each inlineTokens(text, selected?.path ?? '', knownPaths) as token}
		...
		{:else if token.kind === 'link'}<span
				...
				title="surface target is not present">{token.text}</span
			>
		{:else}{token.text}{/if}
	{/each}
{/snippet}
...
<div class="panel p-4">
	<div class="mb-3 flex items-center justify-between gap-3 text-sm">
		<span class="font-mono font-medium tracking-wide text-amber-200 uppercase">work surface</span>
	...
	{#if data.files.length === 0}
		<p class="text-sm text-stone-500">No authored surface mirrored yet.</p>
	{:else}
		<div class="grid gap-3 md:grid-cols-[minmax(11rem,0.28fr)_minmax(0,1fr)]">
			<nav class="subpanel h-fit p-2" aria-label="Work-surface files">
