...
test('the live-demo CTA targets the honest public route and discloses what it shows', async () => {
	const html = await renderLanding();
	ok(html.includes('href="/new?demo"'), 'links to the demo-flagged route, not bare /new');
	ok(html.includes('explore the live demo'));
	ok(
		html.includes('scripted replay of real resident telemetry'),
