<script lang="ts">
	// THE MAIN UI, as a component. This was `src/routes/+page.svelte` until
	// 2026-08-31; it moved here so `/daily` could wear it whole instead of
	// keeping a second, thinner interface beside it (maintainer: "have on the
	// daily the whole main user interface, the visual, like exactly as it
	// is"). `/` and `/daily` are both two-line routes over this file now.
	//
	// One prop is the entire difference between them, and that is the design:
	// the two pages are the same page, and a second copy of 2,100 lines is a
	...
	interface Props {
		/**
		 * How the live-runs slot behaves — the one seam between `/` and `/daily`.
		 *
		 * - `field` (`/`): `ResidentField`, the live body drawn as a body.
		 *   Pressing a cell selects that run and the overlay answers with its
		 *   node panel. Unchanged from before the extraction.
		 * - `map` (`/daily`): the compacted presence bars over the ascii field,
		 *   and a press on either opens that field full-screen with a collapse
		 *   back. Nothing else on the page changes — a pick in the lane still
		 *   opens its run's node exactly as it does on `/`, because the ask was
	...
	// The map's own stage (`liveView === 'map'` only): the same overlay, one
	// step wider, holding the ascii field at full height. Separate from
	// `runOverlayOpen` on purpose — they are two different questions ("show me
	// this run" vs "show me the room"), and only one of them can be asked on a
		...
		<section class="ignite" style="--ignite-delay: 260ms" aria-label="the machine's lane">
			{#if machineExpanded}
				<div in:glitchReveal={{ duration: 240 }}>
					...
					<!-- `liveView === 'map'` (`/daily`) puts the room's own map in this
				     slot instead, with the compacted presence bars over it. It
				     renders whether or not anything is burning — the field is the
				     *scene*, and a still room is a thing the map can say, where an
				     empty peer grid is just nothing. The inline copy stands down
				     while the stage is up: two `AsciiField`s mounted at once means
				     two 2s polls and two cameras disagreeing about where the reader
				     is looking, for a field nobody can see behind the overlay. -->
					{#if liveView === 'map'}
						{#if !mapStageOpen}
							<div class="mt-2">
								<LiveMapField
									runs={liveRuns ?? []}
									stale={liveRunsStale}
									onExpand={() => (mapStageOpen = true)}
								/>
							</div>
						{/if}
					{:else if (liveRuns?.length ?? 0) > 0}
						<div class="mt-2">
							<ResidentField
								runs={liveRuns ?? []}
								stale={liveRunsStale}
