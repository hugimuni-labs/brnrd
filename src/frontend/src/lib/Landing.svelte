<script lang="ts">
	...
	// The /login redundancy (his ask, 2026-08-06): a visitor who taps "sign
	// in" here has just read what brnrd is — /login's own copy repeats that
	// pitch almost verbatim before the real CTA. So this page prefetches the
	// same context /login would fetch, and once GitHub OAuth is confirmed
	// live both sign-in affordances below jump straight to the handshake,
	// skipping the re-pitch. `null` (not yet loaded, or the fetch failed)
	// and `oauth_ready === false` (self-hosted with no OAuth client
	// configured) both fall back to /login unchanged — it still exists, and
	// it still earns its keep for cold deep links that carry their own
	// `next` (a pairing URL) or the disabled-OAuth explanation.
	...
	onMount(async () => {
		...
		// login-context throws on failure (unlike the counters, which swallow
		// their own errors); catch it locally so one flaky fetch never blanks
		// the counters this Promise.all otherwise resolves independently.
		const [s, r, doors, login] = await Promise.all([
			fetchPublicStats(),
			fetchRepoStats(),
			fetchDoorStatus(),
...
<div class="mx-auto max-w-4xl p-6">
		...
		<div class="mt-6 grid grid-cols-1 gap-8 lg:grid-cols-[minmax(0,1fr)_380px] lg:items-start">
			<div class="order-2 max-w-2xl lg:order-1">
				...
				<p class="mt-3 text-sm leading-relaxed text-stone-400">
					Your models, your hardware, your agent CLI — signed in with the Claude Code or Codex
					subscription you already pay for (an API key works too). That's the whole product,
	...
	<section class="ignite mt-10" style="--ignite-delay: 380ms" aria-label="shells and doors">
		...
		<div class="mt-3 grid grid-cols-1 gap-4 md:grid-cols-2 md:items-start">
			<div class="panel p-4">
	...
	{#if !countersLoaded || stats !== null || repo !== null}
		<section class="ignite mt-10" style="--ignite-delay: 450ms" aria-label="live counters">
			...
			<p class="mt-2 font-mono text-sm text-stone-400">
				{#if stats !== null}
					<span class="text-amber-100">{stats.accounts}</span> account{stats.accounts === 1
						? ''
						: 's'}
				{/if}
