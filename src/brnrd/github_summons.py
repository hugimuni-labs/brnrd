...
def _is_own_machinery(payload: dict) -> bool:
    """Is this a pull request the resident's own strand fleet opened?
    ...
    strand push and therefore indistinguishable from a human's.
    """
...
def resolve_github_summons(
    ...
    if spec.target_source == "mention":
        if spec.action == "opened" and _is_own_machinery(payload):
            # A PR the resident's own fleet just opened. Strand branches live
            # in brnrd's own ``brr/`` namespace and strand commits are pushed
            # under the operator's identity, so the author-skip below cannot
            # see them: a strand PR whose description @-mentions the bot
