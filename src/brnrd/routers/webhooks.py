...
from ..platforms import signal as sig
...
def _signal_signature_ok(secret: str, raw: bytes, supplied: str | None) -> bool:
...
def _signal_reply(settings, parsed: sig.ParsedMessage, text: str) -> None:
    if not (
    ...
    except Exception as exc:  # noqa: BLE001 - provider refusal is logged, not a 500 retry storm
        logger.warning("signal reply failed: %s", exc)


def _signal_route(db: Session, parsed: sig.ParsedMessage) -> ChannelRoute | None:
    return db.execute(
        select(ChannelRoute).where(
            ChannelRoute.platform == "signal",
            ChannelRoute.channel_id == parsed.chat_id,
        )
    ).scalar_one_or_none()


def _handle_signal_pair(
    db: Session, settings, parsed: sig.ParsedMessage, code: str
) -> None:
    pc = db.execute(select(TgPairCode).where(TgPairCode.code == code)).scalar_one_or_none()
    expires = pc.expires_at if pc else None
    if expires is not None and expires.tzinfo is None:
    ...
    if (
        ...
        _signal_reply(settings, parsed, _PAIR_CODE_EXPIRED_TEXT)
        return
    if pc is None or pc.consumed:
        _signal_reply(settings, parsed, "Invalid or expired pair code.")
        return
    existing = _signal_route(db, parsed)
    if existing is not None and existing.account_id != pc.account_id:
        _signal_reply(settings, parsed, "This chat is already paired to another account.")
        return
    display = parsed.user or parsed.chat_id
    if existing is None:
    ...
    target = _route_target_repo(db, existing)
    tail = (
        f" I'll route messages to the right project (currently '{target.repo_full_name}')."
        if target is not None
        else " " + _NO_REPO_YET_TEXT
    )
    _signal_reply(
        settings,
        parsed,
        f"Paired with {login}'s brnrd account — this Signal chat now reaches your resident.{tail}",
    )
...
async def signal_webhook(
    request: Request,
    x_brnrd_signal_signature: str | None = Header(default=None),
):
    """Authenticated ingress from the persistent Scaleway Signal bridge."""
    settings = request.app.state.settings
    raw = await request.body()
    if not _signal_signature_ok(
        settings.signal_webhook_secret, raw, x_brnrd_signal_signature
    ):
        raise HTTPException(status_code=status.HTTP_403_FORBIDDEN, detail="bad signature")
    try:
        ...
        return {"ok": True}
    parsed = sig.parse_update(payload) if isinstance(payload, dict) else None
    if parsed is None:
        return {"ok": True}
    with request.app.state.SessionLocal() as db:
        code = _bare_pair_code_from_text(parsed.text)
        if code:
            _handle_signal_pair(db, settings, parsed, code)
            return {"ok": True}
        route = _signal_route(db, parsed)
        if route is not None and _message_precedes_route(parsed, route):
            return {"ok": True}
        if route is None:
            _signal_reply(settings, parsed, _WA_UNPAIRED_TEXT)
            return {"ok": True}
        repo = _route_target_repo(db, route)
        if repo is None:
            _signal_reply(settings, parsed, _NO_REPO_YET_TEXT)
            return {"ok": True}
        decision = limits.check_event_admission(
            db,
            settings,
        ...
        if not decision.allowed:
            _signal_reply(settings, parsed, decision.message)
            return {"ok": True}
        inbox_service.enqueue(
            db,
            repo_id=repo.id,
