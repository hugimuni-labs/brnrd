...
#: The event ``source`` a concurrently-dispatched ``spawn:`` strand records.
#:
...
#: dispatched down the strand stack, not woken as this resident's next thought.
_SPAWN_SOURCE = "spawn"


def _dispatched_as_strand(score_path: Path) -> bool:
    """Was the run that wrote this score a concurrent ``spawn:`` strand?
    ...
    **``spawn:`` and ``respawn:`` are distinguishable here, and the distinction
    ...
    respawn is the same thought continuing in a different body.  A *strand*
    that respawns does inherit ``"spawn"`` and stays skipped — also right, it
    is still not this resident's line.

    Anything this cannot prove is a strand degrades to "not a strand": an
...
def find_prior_wake(
    ...
    *Whose* previous self is the part this originally got wrong (#987).  Newest-
    first with no notion of parentage meant a ``spawn:`` strand the resident had
    ...
    ``continuity: ✓ run-260802-0649-lcrd`` (a codex strand) while its own
