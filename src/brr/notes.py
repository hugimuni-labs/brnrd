...
_REGISTRY: tuple[Surface, ...] = (
    ...
    Surface(
        key="linger-opt-out",
        root=ROOT_INTRA_RUN,
        path_hint="<outbox>/.linger-opt-out",
        role="consciously end a live conversation without the default linger",
        readers=("brr.hooks._linger_opted_out",),
        grammar="first non-empty line is the reason",
        parser="brr.hooks._linger_opted_out",
        lifetime=LIFETIME_RUN,
    ),
    Surface(
        key="promises",
