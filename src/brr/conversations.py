...
delivery and skips a second run, the same exchange can appear on
...
def _correspondent_dedup_identity(record: dict[str, Any]) -> tuple[str, ...] | None:
    ...
    already seen and skips a second run (``daemon.py``'s
...
def _dedupe_woven_records(
    ...
    recognises the mirrored delivery and skips a second run. Left
