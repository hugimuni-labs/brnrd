    ...
    try:
        ...
        payload: dict[str, object] = {
            ...
            "card": {
                ...
                # move 5b (§19.2): the frame-drafted "since your last card
                # write" item — `note: <id>` accepts it, a card edit folds it.
                "delta": card_frame.portal_delta(task.meta),
            },
            # move 5b (§20): the lit heddles, brightest first, computed on
            # the heartbeat (`_frame_heartbeat`) — a new key, so move 5's
            # HUD rewrite (#1976) carries it as one more field.
            "heddles": list((card_state or {}).get("heddles") or []),
