...
# No wake-tap horizons here any more (#733). A dashboard tap's claim window
# and staleness backstop were local replicas of a lifecycle the server owns,
# and the two config knobs that tuned them (`dispatch.wake_request_ttl_seconds`,
# `dispatch.wake_request_claim_window_seconds`) tuned a second opinion. The
# server answers at `_apply_dashboard_wake_request`; see `wake_request.py`.
...
def _wake_reason(value: object) -> str | None:
    """Squash a server refusal reason into one safe frontmatter line.

    Event frontmatter is flat and line-oriented (``protocol.update_event_meta``
    formats values with ``str()``), so a newline in a value would split the
    block. The reason is server-authored and short in practice; this is the
    guard that keeps "in practice" from being the load-bearing part.
    """
...
def _stamp_wake_request(
    ...
    back rather than ask again.

    ``dashboard_wake_request_reason`` present ⇒ refused. That is the whole
    ``applied`` bit; a separate boolean key would be a second copy of the
    same fact, and this change is about not keeping those.
    """
    updates: dict[str, object] = {"dashboard_wake_request_id": request_id}
    if profile:
        updates["dashboard_wake_request_profile"] = profile
    if reason:
        updates["dashboard_wake_request_reason"] = reason
    return updates
...
def _apply_dashboard_wake_request(
    ...
    (#564), profile availability, and #577's parked-after-the-event rule —
    now lives in ``brnrd/wake_requests.claim`` beside the row it judges, and
    the answer arrives with that row already at its final status. The daemon
    ...
    An event-level runner pin (``respawn shell:``/``core:``, ``quality:
    escalate``) still outranks the tap and short-circuits *before* the
    claim — deliberately, so the tap stays pending for the next unpinned
    wake instead of being spent on one that was never going to honour it.
    """
    event = target.event
    ...
    verdict = _cloud.claim_wake_request(
        brr_dir,
        request_id=request_id,
        event_id=event_id,
        source=source or None,
        event_created=str(event.get("created") or "") or None,
    )
    if verdict is None:
        # Fail-open, and byte-identical to the empty-mirror path above: an
        # unreachable brnrd.dev costs the tap, never the wake. No receipt
        # either — nothing was decided, so claiming otherwise would be the
        # invisible-outcome bug in a new place.
        return target

    if str(verdict.get("status") or "pending") != "pending":
        # The server just told us the row is retired; don't wait a publish
        # tick to stop offering it to the next dispatch.
        wake_request_mod.drop_pending(brr_dir)

    claimed_id = str(verdict.get("request_id") or "").strip() or request_id
    profile = str(verdict.get("profile") or "").strip()
    repo_label = str(verdict.get("repo_label") or "").strip()

    def refuse(reason: str) -> _DispatchTarget:
        wake_request_mod.record_receipt(
            brr_dir, claimed_id, source=source, event_id=event_id,
            profile=None, outcome="refused", reason=reason,
        )
    ...
    if not profile:
        return refuse("the server applied a tap that named no profile")

    if repo_label:
        repo = account_context.repo_for_label(repo_label)
        if repo is None:
            # The dashboard validates ``repo_label`` at mint time, so this is
            # a rack that has drifted from the daemon's registered repos. The
            # row is already spent server-side and we can't route the wake
            # where it asked to go — running the requested profile against
            # the *wrong* repo would answer a question nobody asked, so
            # refuse and let the receipt say why.
            return refuse(f"repo '{repo_label}' is not registered on this daemon")
        target = _DispatchTarget(
            event=event,
            repo_root=repo.root,
    ...
    updates = _stamp_wake_request(event, claimed_id, profile, None)
    updates["runner"] = profile
    if repo_label:
        updates["repo_label"] = repo_label
    environment = str(verdict.get("environment") or "").strip()
    if environment:
...
def _run_worker(
    ...
    *,
    ...
    The body lives in :mod:`brr.worker` — the throw, in five phases
    (prepare · dispatch · stream · boundary · finalize) with a typed shape
    at every seam. This name and signature stay so every caller, and every
    test that patches ``daemon._run_worker``, is untouched.
    """
    from . import worker
...
def _processing_event_is_orphaned(
    ...
    *,
    ...
    if not _run_unfinished(manifest):
        ...
        return True
    if _recorded_pid_alive(manifest):
        return False
    try:
...
def _pending_events_for_agent(
    ...
    *observer_run_id*, when given, is **this call's own run id** — this
    ...
    _live_run_ids_for = _live_run_ids_cache(now=now_ts)
...
def _write_live_portal_state(
    ...
    *repo_label* is the caller's single repository-identity fact, shared with
    ...
    The body moved to :mod:`brr.hud` (move 5 of the daemon rewrite): the
    parameters are one :class:`hud.HUDInputs`, the payload is one typed
    :class:`hud.HUD`, and this stays the patchable entry point every caller
    reaches by name.
    """
    from . import hud

    return hud.write_live(hud.HUDInputs(
        outbox_dir=outbox_dir,
        inbox_dir=inbox_dir,
...
_NOTICE_KINDS = _outbox_notices.KINDS
...
_NOTICE_LIFETIMES = _outbox_notices.LIFETIMES
...
def _record_outbox_notice(
    ...
    source_file: str = "", verb: str = "", run: str = "",
) -> None:
    ...
    *source_file*, when given, is the staged directive's own filename
    ...
    Explicit today from the ``cut:``/``hold:`` branches, the entry guard and
    the staging-casualty drop; every *other* notice written while the drain
    is handling a staged file inherits the file, the verb that claimed it
    and the run from ``outbox.notices.attributed`` (move 4). A notice
    written outside a drain carries none of the three.

    The row itself is written by ``outbox.notices.write`` — the one writer;
    this name stays because ~70 call sites and their tests reach it here.
    """
    _outbox_notices.write(
        kind, text, outbox_dir=outbox_dir, lifetime=lifetime,
        source_file=source_file, verb=verb, run=run,
    )
...
def _drain_outbox(
    ...
    *,
    ...
    The dispatch lives in ``brr.outbox`` (move 4): ``table.py`` holds the
    precedence, ``verbs.py`` one handler per frontmatter key, ``drain.py``
    the listing and parse that run before any verb. This name and signature
    stay so every caller and ``monkeypatch.setattr(daemon, "_drain_outbox",
    …)`` keeps landing.
    """
    from .outbox import drain as outbox_drain

    return outbox_drain.drain(
        emit, task, responses_dir, event_id, outbox_dir, inbox_dir,
        repo_root=repo_root, account_context=account_context, stats=stats,
    )
...
def _reap_zombie_run_state_docs(
    ...
    *,
    ...
    for registered in account_context.repos.values():
        live_run_ids |= _live_run_ids(
            gitops.shared_brr_dir(registered.root), now=timestamp,
        )
...
def _live_run_ids(brr_dir: Path, *, now: float | None = None) -> set[str]:
    """Run ids with a live presence entry under *brr_dir* right now.

    The one comprehension every boot-time sweep (and the resident's own
    pending-mail view) needs before it can call a stuck run "provably
    dead" — presence is the live authority the whole proof ladder in this
    module starts from. Extracted because it had drifted into four
    independent hand-rolled copies (the run-manifest janitor, the
    account-state-doc janitor, ``_mark_interrupted_runs``, and
    ``_reconcile_orphaned_spawn_dispatches``'s own cache) plus a fifth in
    ``_pending_events_for_agent``'s local closure — a fact re-derived five
    times gets fixed once and drifts four times.
    """
    return {
        str(entry.get("run_id") or "")
        for entry in presence.list_active(brr_dir, now=now)
    }


def _live_run_ids_cache(*, now: float | None = None):
    """A memoizing ``brr_dir -> live run ids`` lookup, scoped to one sweep.

    Building the presence-active set is a directory scan; several sweeps
    below ask for it once per event or per task rather than once per
    unique ``.brr`` dir, so this collapses repeat scans of the same dir
    within a single pass. Same cache shape ``_reconcile_orphaned_spawn_
    dispatches`` and ``_pending_events_for_agent`` used to hand-roll
    independently (``dict[Path, set[str]]`` + a manual ``if brr_dir not in
    cache`` guard each) — one memoizer, two call sites.
    """
    cache: dict[Path, set[str]] = {}

    def get(brr_dir: Path) -> set[str]:
        cached = cache.get(brr_dir)
        if cached is None:
            cached = _live_run_ids(brr_dir, now=now)
            cache[brr_dir] = cached
        return cached
...
def _record_retry_provenance(event: dict, run_id: str, failure_kind: str) -> None:
    ...
    paired lists — ``retry_of`` (run ids) and ``retry_failure_kind`` (the
    ...
    Named ``retry_failure_kind``, not the ``retry_reason`` #1500 first wrote
    here: ``RunnerResult.retry_reason()`` (``runner.py``) already owns that
    name in this same module, for an unrelated fact — *why the current run
    will be retried in-loop*, not *what a prior interrupted run died of*.
    Reconciling #1495/#1500 renamed the newer, narrower fact so a reader (or
    a grep) cannot conflate the two.
    ...
    if run_id in existing_of:
        return
    existing_kind = [
        r for r in str(event.get("retry_failure_kind") or "").split(",") if r
    ]
    existing_of.append(run_id)
    existing_kind.append(failure_kind)
    protocol.update_event_meta(
        event,
        retry_of=",".join(existing_of),
        retry_failure_kind=",".join(existing_kind),
    )
...
def _mark_interrupted_runs(
    ...
    *,
    ...
    meant to stay exactly as it was. That paragraph is no longer true, for
    two distinct reasons this sweep now handles side by side:

    - **Visibility** (#1496, "the event nobody could see"): the still-
      ``processing`` event genuinely stays dispatch-eligible
      (``protocol.list_dispatchable`` treats ``pending``/``processing``
      alike), but the *resident's own* pending view
      (``_pending_events_for_agent``) filters strictly on
      ``status == "pending"`` and never treated ``processing`` as visible —
      so an orphaned event dispatched to a run this sweep just marked
      interrupted sat dispatchable-but-invisible to any resident that
      folded in pending mail before the retry actually landed, for however
      long the retry took to queue. A retry-eligible orphaned event now has
      its status flipped back to ``pending``. That is still "the retry path
      stays exactly as it was" in the sense that matters — ``pending`` is
      exactly as dispatch-eligible as ``processing`` — it just stops being
      invisible while eligible. **Carved out:** an orphaned *spawn*
      dispatch (``spawn_immediate`` + ``spawn_parent_run_id``, no
      ``spawn_message_for_event``) is left at ``processing`` untouched —
      that shape is ``_reconcile_orphaned_spawn_dispatches``'s own
      territory (below, boot-ordered right after this sweep), which
      requires ``status == "processing"`` to find the event and notify the
      parent of the crash. Flipping it to ``pending`` here first would make
      that sweep silently skip it, losing the crash notification and
      letting the event re-surface as a plain dispatchable event that
      quietly re-spawns a duplicate strand instead.
    - **Provenance** (#1491): whenever the event is still dispatchable
      (spawn carve-out included — the stamp is additive and does not
      affect dispatch), this sweep appends this run's id and failure kind
      to the event's own ``retry_of``/``retry_failure_kind`` history, additively
      — see :func:`_record_retry_provenance` — so the run (or, for a
      carved-out spawn dispatch, the notified parent) can say *why* this
      is a retry, not just answer cold.
    ...
    # the honest basis for the card's "retrying" tail, and (below) both what
    # the visibility fix flips back to "pending" and what #1491's
    # retry-provenance stamp writes onto. Keyed to the live event dict — not
    # just its id — so both writes land straight on it without a second
    # disk read.
    ...
    for root in roots.values():
        ...
        for task in list_runs(runs_dir):
            ...
            if will_retry:
                ...
                # Fires for a carved-out spawn dispatch too — the stamp is
                # additive and does not touch dispatch state, unlike the
                # status reset just below.
                try:
                    _record_retry_provenance(
                        retry_event, task.id, runner_failures.HOST_INTERRUPTED,
                    )
            ...
            if (
                ...
                # #1496 ("the event nobody could see"): reset to "pending"
                # unless `_reconcile_orphaned_spawn_dispatches` (below) owns
                # this exact shape instead — see this function's own
                # docstring.
...
def _reconcile_orphaned_spawn_dispatches(
    ...
    *,
    ...
    live_run_ids_for = _live_run_ids_cache(now=timestamp)
    ...
    for target in _dispatchable_targets(account_context, repo_root, cfg):
        ...
        if any(t.id in live_run_ids_for(brr_dir) for t in event_runs):
            continue
...
def _undefer_held_event(
    ...
    *resume_native_session_id* is the seat's scroll, and it is the caller's
    to hand over — never this function's to assume. It used to be passed for
    every sibling in the drawer ("one of them might lead the dispatch"),
    which made one release write N claims on one transcript, each surviving
    on disk long past the release (brnrd#2022: a 2026-09-17 correspondent
    message came out of a drawer carrying a session id minted the next day,
    and was retried as a fresh `claude --resume`). Today
    ``_apply_resource_hold_resume`` passes nothing — the releasing event is
    the carrier — and the measured-refill path, which has no releasing
    event, passes it for exactly one sibling.
    """
    if not event_id:
        return
    if inbox_dir is None:
...
def _apply_resource_hold_resume(
    ...
    if wants_fresh:
        # brnrd#2022, the belt to #2016's brace: a handover release does not
        # merely decline to *add* a stamp, it strips whatever the event is
        # already carrying. The stamp is a cache and it heals — deleted from
        # `evt-…-udee` by hand at 15:10Z on 2026-09-18, it was back at 15:14Z
        # when the release re-derived it from the parked run's
        # `resource_hold.native_session_id`. Declining to write covers the
        # paths this daemon owns; stripping covers every path it does not
        # (a mint before this fix, a hand-edited inbox, a future minter that
        # copies frontmatter the way `_queue_respawn_request` did).
    ...
    for accumulated_id in released.get("accumulated_event_ids") or []:
        # brnrd#2022: the releasing event carries the scroll; its accumulated
        # siblings do not. This loop used to stamp every one of them, on the
        # theory that a sibling could sort ahead of the trigger and lead the
        # dispatch instead. The price, measured: `evt-…-2jb8` is a
        # correspondent's message from 2026-09-17 that sat in this drawer
        # through a host suspend and came out carrying a session id minted a
        # *day later*; the daemon retried it as a fresh run and it did
        # `claude --resume` onto a scroll it had no relation to. A stamp is a
        # one-shot claim on one transcript — written onto N events it is N
        # claims, each outliving the release that made it. The honest failure
        # direction is a cold boot when a sibling leads, never a warm boot
        # onto the wrong scroll.  `_rekey_to_seat` still runs: routing home
        # is true of every letter in the drawer, which is why it stays.
        _undefer_held_event(
            drawers, accumulated_id, seat_conversation=seat,
        )
...
def _release_reset_holds_due(
    ...
    for root in roots.values():
        ...
        for held in _held_runs_for_repo(runs_dir):
            ...
            # the releaser, and the drawer is all there is to wake on. So
            # unlike `_apply_resource_hold_resume` above, the scroll has to
            # ride an accumulated sibling or the seat boots cold off its own
            # refill. brnrd#2022 holds the invariant that matters instead of
            # the whole spray: **at most one event ever carries the scroll**.
            # The first accumulated id is the seat's own oldest queued
            # letter, which is the one an ordinary pending sort leads with;
            # if some later sibling leads anyway the seat boots cold, which
            # is the direction this is allowed to fail in.
            drawers = _hold_undefer_inboxes(account_context, inbox_dir)
            for position, accumulated_id in enumerate(
                released_meta.get("accumulated_event_ids") or []
            ):
                carries_scroll = position == 0
                _undefer_held_event(
                    drawers, accumulated_id,
                    resume_native_session_id=(
                        released_meta.get("native_session_id")
                        if carries_scroll else None
                    ),
                    resume_native_provider=(
                        released_meta.get("provider") if carries_scroll else None
                    ),
