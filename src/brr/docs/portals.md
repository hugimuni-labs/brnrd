...
| `<name>.md` with `fetch: issue` or `fetch: pr` and `number: <n>` frontmatter | inbound ◂ forge read | Ask the host daemon to read one issue or PR from **this run's repository** with the GitHub gate credential; the result returns as a pending run-edge event in `inbox.json`. `issue` exposes fixed metadata, body, labels, assignees, and the first 50 comments. `pr` exposes fixed metadata, body, labels, assignees, review decision, the first 100 changed-file summaries (path/change type/additions/deletions), first 50 conversation comments, and first 50 reviews with their first 50 inline comments. These two immutable projections are the allowlist: the request cannot supply a body, URL, REST path, GraphQL document, HTTP method, hostname, owner, pagination cursor, patch-content selector, or search. Optional `repo:` is only a same-repo assertion; another repo is refused. Responses are capped at 64 KiB and each run gets four requests. Every refusal returns as a pending event **and** lands in `notices`. |
...
`portal-state.json` → `notices` is where those land. A `fetch:` refusal also
returns as a run-edge event, so the request has both immediate and persisted
visibility. **Read it after any `fetch:` / `spawn:` / `respawn:` / `event:`-addressed
