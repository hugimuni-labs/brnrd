...
- `✓` — the prior wake's boot-score.json was found and parses (the mount holds).
  "Prior wake" means the previous thought on *this resident's line*: a score
  recording `source: spawn` belongs to a strand this line dispatched, not a
  predecessor, and the picker skips it. A `respawn:` handoff inherits the
