...
`--timeout` = the ceiling · default = this run's own remaining budget (the daemon knows it; restating it is the mistake `spawn:<id>` was) — **no configured budget ⇒ no default ceiling either: the seat stays open**, resolved only by a pending event (a configured hard budget cap still bounds it, and the daemon says so as an `advisory` notice when it does) · under the porcelain this is `timeout: none`, not an omitted key — a missing `timeout:` stays refused
...
`"timeout"` the deadline passed with nothing else firing — never fires at all on an open-ended wait unless a hard budget cap gives it a deadline to reach
...
`.keepalive` is extended for you, to the timeout deadline (never shortened below one you wrote yourself) — no separate keepalive write on top of the wait · no deadline (open-ended wait, no hard cap configured) ⇒ nothing to extend: a run with no configured `runner.timeout_seconds` never reaches the budget check the keepalive exists to outlast
...
under the porcelain = one outbox directive (`await: true` + `timeout:` — a duration, or `none` for no ceiling · optional `file:`) · `brnrd await` stages it for you
a directive still carrying v1's conditions ⇒ refused with a notice naming `brnrd await` — never silently, never by ignoring the extra terms · malformed input (no `timeout:` at all — an omission, not a decision) ⇒ `notices`, same as any other verb
