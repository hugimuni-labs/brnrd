...
- **`--item <id>` binds the immediately preceding `--reply` to a warp item**
  (design-the-water-line.md §The asks lane, rung 2 — the join the rail
  rendering, rung 3, is blocked on). Repeatable: one reply may answer
  several asks. Checked client-side before anything is staged, same
  "nothing was staged" guarantee as the promise-debt contract above: an id
  that doesn't resolve (`items.resolve_item` — the same exact-id authority
  `brnrd item list` is built on, no fuzzy headline match) refuses the whole
  call, naming the id; `--item` with no preceding `--reply`, or trailing a
  `--gate` (which has no event to bind), is a pairing error, refused the
  same way `--body-file`-with-no-verb already is. Existence is the only
  gate — a binding to a `done:`/`retired:` item is legal, unlike weld's
  ignition claim, since an ask binding records a conversation fact, not a
  claim that work is starting. The write is **per reply, not per call**,
  unlike `--promise`'s one row per batch: each staged reply's own drain
  verdict gates only its own item rows, so one refused reply in a
  multi-`--reply` call never blocks a sibling reply's bindings. Accepted
  rows land in `.asks.jsonl` (`do.append_ask`) — dot-prefixed like every
  other control file in this directory (`.relics.jsonl`,
  `.promises.jsonl`), since a bare `asks.jsonl` would sit where
  `_drain_outbox` scans for deliverable messages and only a dotfile is
  exempt from that scan. Each row is `{"event": <id>, "item": <id>}` — the
  design page's own "evt → item → reply-evt" shorthand collapses to two
  fields here: the outbox grammar has no separate reply-event id, since an
  `event:` reply targets and closes the same event id it names, so there is
  no third id to record.
