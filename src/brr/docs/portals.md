...
- `resume: any` — the seat's resting state (design-the-seat-that-never-quits.md): a correspondent message, an own strand, or a `schedule` firing releases it. `seat.park_on_turn_end: true` in config makes the daemon choose it for every user-woken seat whose turn ends with nothing armed — `held`, never `done`; off by default until signed
