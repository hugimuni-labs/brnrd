...
| `<name>.md` with `spawn: true` frontmatter | concurrent ↗ strand dispatch | Queue a strand-stack child. Admission reads the binding quota floor at each dispatch: no floor starts now, `low` queues FIFO and emits `spawn_queued`, `critical` refuses. `portal-state.json` → `resources.coexisting_runs.spawn_pool` reports the floor and queued count. Name `shell:` / `core:`; `worktree` is the isolation floor. Completion returns pending to the parent. |
| `<name>.md` with `submit: true` frontmatter | concurrent ↗ strand review | From a live strand, verify its declared branch and report, emit `spawn_submitted` with produce handles to the parent, and leave the strand alive. A missing contract fact is refused; another accepted submit after a steer emits a new generation. Follow with bare `brnrd await`; the parent releases with `stop:`. |
...
| `<name>.md` with `config_change: <key>` frontmatter | parked ⏸ policy approval | Propose raising an allowlisted integer config ceiling in `.brr/config` (dominion injection budgets). Required `value:` carries the requested value; optional `repo:` scopes it. The daemon parks the proposal for account-owner approval; no config changes until accepted. |
