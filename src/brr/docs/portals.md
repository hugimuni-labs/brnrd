...
**the lease (move 2c)** — one call holds until the daemon resolves the wait or the lease ceiling passes (`--ceiling <duration>`, at most 6h; default 50m — or the run's remaining budget when shorter — because a cache read of the scroll is weighted 0.1 and a cold write 1.25: an hourly warm re-read beats one cold wake until ~12.5h of silence, measured against the seat's 2026-09-13 night of 48 re-calls) · the ten-minute return is gone as a return condition · what bounds a call is the Shell's per-tool-call cap, and on claude that is `BASH_MAX_TIMEOUT_MS` (default 600000 — at the cap claude 2.1.269 *backgrounds* the command and hands the turn back, which in `-p` is a quit) · so: the daemon widens `BASH_MAX_TIMEOUT_MS` for claude runners to 6h10m (an operator's own value is kept) · the pre-tool hook matches `brnrd await` Bash calls, sets `timeout` to the Shell's maximum and exports that number as `BRNRD_AWAIT_CALL_CAP_MS` into the call · the CLI returns 45s under a stamped cap it cannot outlive (`shell_cap`), and falls back to the per-Shell slice (claude 560s, else 480s) when nothing stamped one — codex, a hookless profile · every return leaves `.await-lease.json` in the outbox and the first post-tool boundary after renders `slept <d> · woke: <outcome>` once (the `lease_wake` DELTA segment) · while the lease holds there are no tool boundaries — the point — and the seat is still a current thought: `run_daemon`'s dev-reload waits on `current is None and not active_spawns`, and a leased resident's runner future is not done · the old 15s bound is gone with the argument that justified it — the only things that would want to interrupt this run are the very things that resolve the wait ⇒ a blocking call returns the moment one arrives
