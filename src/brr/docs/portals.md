...
| `.topic` | slot control | **This run's one topic** (design-the-loom §21, move 5c) — first line: an existing heddle's slug, `new <slug>` (minted by the frame; a resident only), or `null`. Set at boot beside `.card` and `.mood`. The waking event arrives with `topic_proposed` (the signatures' best match over its text, else the thread's last assigned topic); the first `.topic` that resolves confirms or overrides it — the event gains `topic:` and one `event` row lands in `surface/topics/<slug>.index.jsonl`. Every outbox act (`event:` replies, `cut:`, `fold:`, `land:`, `spawn:`) inherits it unless the act carries its own `topic: <slug>`; nothing is reclassified after the fact. A run that ends in `error` with no topic is marked `topic_unset`, and the next wake on the thread may `topic: assign <slug> -> <event-id>` once. A control file, never delivered. |
