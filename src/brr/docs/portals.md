...
| `<name>.md` with `gate: <name>` frontmatter | outbound ▸ a destination | A **send** to a destination with no waiting event — ping a chat, post out-of-band, deliver from a scheduled wake. A destination is written **`carrier:app`** — `host:telegram` is the bot token on this machine, `cloud:telegram` is the relay into the maintainer's thread — so no name can mean two things. (`carrier` = who moves the bytes: `host` or `cloud`. `app` = where it lands: `telegram`, `whatsapp`, `github`, `slack`, `signal`. It is the shape conversation keys have always had: `cloud:telegram:155783668:` is carrier, app, who.) A **bare** name that is both a host gate and a live cloud channel is refused, naming both candidates; a bare name with no collision still resolves as before. `gate:` names a **transport**, not a person, and transports are named after the app their correspondents use — so the most tempting value is the one that names the platform and misses the reader. `gate: forge` is the explicit PR handoff (`head`, `base`, `title` frontmatter; body is the PR body) and opens or refreshes the PR for that head branch when the GitHub gate can deliver. Diffense may generate the title/body when a checked review pack exists, but PR delivery is not diffense-owned. An unconfigured gate is dropped. The body is close-keyword checked before the PR is queued — see **Close keywords close on two channels** below. Optional `thread:` frontmatter is recorded but only routes the send when the resolved gate declares it can (`CAN_ROUTE_THREAD`) — no built-in gate does today, so it lands on the gate's own default destination and an `advisory` notice names where; `event: <id>` is the lane that actually routes to a specific thread. |
...
| `<name>.md` with `spawn: true` frontmatter | concurrent ↗ strand dispatch | Queue a strand-stack child. Admission is **never a quota decision**: a dispatch tick that sees the strand starts it, whatever the binding floor reads. (It used to gate on `pacing.quota_low_floor_pct` — `low` queued the child FIFO, `critical` refused it. That number was written to stretch `every:` schedule intervals, and borrowed here it closed the cheap lane — a strand with its own allowance and a fresh context — while leaving the expensive resident seat running. The floor governs how much is spent, never which lane spends it.) `spawn.max_concurrent` is the only width gate. Read `portal-state.json` → `resources.coexisting_runs.spawn_pool` (`floor`) for the live scarcity to *pace* by — information, not permission. Name `shell:` / `core:`; optional `environment:` may opt the child *down* the isolation ladder (`docker` / `solitary` — both keep the child's own worktree, so the parent-collision guard holds); `worktree` is the default and the floor, and anything less isolated (`host`) is refused with a notice. Optional `allowance: <tokens>` (design-the-allowance.md, slice 1) — an integer, or a `k`/`m` suffix (`120k`, `1.2m`) — stamps the child's own token budget; unset resolves `spawn.allowance_tokens` from config at dispatch (default 20m), never cached, and a value that doesn't parse refuses the whole spawn with a notice naming it. The unit is cost-weighted tokens (cache re-reads at 0.1, cache writes 1.25, output 5, fresh input 1 — the providers' price ratios; a raw sum is ~100x the bill on any cached session). The daemon meters the child's own live spend at every heartbeat (`portal-state.json` → `resources.allowance`: `{tokens, spent, pct}`) and its bar renders `spend 38k/120k` in place of the shared quota chip; at ≥100% the next boundary carries one directive, once, until the number moves — park (`submit:` + `brnrd await`) or `ask: allowance +<tokens>` (see below), never a kill. The completion returns to the parent as a pending event. The parent retains ownership of any original external event and answers it with `event: <id>` after reviewing the child; the spawn request alone does not clear that event. The resident seat's own run gets the same facet shape under a different scope (design-the-allowance.md §2, slice 2): `resources.allowance` reads `{tokens, spent, pct, scope: "resident"}`, `tokens` is config-owned (`resident.allowance_tokens`, same default as `spawn.allowance_tokens`) rather than a `spawn:`-declared ceiling, and `spent` resets to zero when the binding quota's own reset instant advances (a window roll), not on any local clock. Unlike a strand, the resident's bar shows this chip *beside* the ordinary quota chip rather than in place of it, and an overrun renders no park/ask directive — `submit:`/`ask: allowance` are strand-only verbs a resident run cannot use. The shared quota gauge itself gains an attribution (brnrd#1810, design-the-seat-that-never-quits.md §"The measurement"): `resources.quota.draws` — `{"self": <this run's own weighted spend, or null>, "strands": [{"run_id", "title", "weighted"}]}` — a strand's `weighted` is `null` until its own first heartbeat has metered something, off the same `allowance_spent` value `resources.allowance` already carries for it, never a second meter. The bar renders it beside the `q` chip: `q S83 · me 1.2m · ▷2 3.4m` (self, then the owned strands' summed spend and count) — absent halves render nothing, never a fabricated `0`. |
| `<name>.md` with `submit: true` frontmatter | concurrent ↗ strand review | **Only from a live strand.** The daemon verifies the two contract facts — the declared `branch:` published through the lane it owns (a local head with no commits the remote-tracking ref lacks), the declared `report:` stat-able as a file — then emits `spawn_submitted` to the parent carrying the same produce handles `spawn_completed` does, marks the child `submitted`, and **leaves it running with its worktree intact**. A missing contract fact refuses the whole directive with a notice naming what is missing and emits nothing; fix it and submit again. Each accepted submit is a new `spawn_submit_generation`, so a `to:` steer followed by a second submit reads as a revision rather than a duplicate. Follow a `✓` submit with bare `brnrd await` — `to:` resolves that wait, `stop:` releases the strand and returns the last submitted attestation as `spawn_completed` with `status: released`. Until then the child is *live*: the parent's own bolt bounces on it like any other undispositioned strand. |
...
| `<name>.md` with `to: <run-or-event-id>` frontmatter | concurrent ▸ strand steer | Message a concurrent child **this run** dispatched (same ownership check as `stop:`). The body lands as a `dispatch_message` event that **only the addressed strand's** `inbox.json` / portal-state surfaces — it never dispatches a run of its own, other runs never see it, and whatever the child has not folded in is retired when the child ends. A steer, not a new contract: the child folds it into its existing work and should not `event:`-address it. Strands are thread-isolated (they get their contract and these edge messages, not the user thread's recent turns or pending events), so this verb is the *only* way words reach a running strand. When the body's first line is `allowance: +N` (additive) or `allowance: N` (absolute), the daemon also raises the live ceiling on the target strand's own run-control record — mechanical, not left to the child to notice in prose — before delivering the same body as the ordinary steer. Refusals land in `notices`. |
| `<name>.md` with `ask: allowance +<tokens>` frontmatter | concurrent ▸ strand ask | **Only from a live strand** (design-the-allowance.md §2, slice 1). At or past its own `spawn:`-declared allowance, a strand asks its parent for more instead of parking: the body is the one-line why. Lands `spawn_allowance_requested` on the parent, carrying the strand id, its last-metered spend/allowance, and the delta requested. A non-strand caller, a non-positive/unparsable delta, or an empty body all refuse with a notice naming why; nothing is queued. The parent answers with `to: <id>` carrying an `allowance:` grant (see above) or `stop:` (release, keeping the last `submit:`). Asking — regardless of whether the parent ever grants — exempts the strand from the cut-time allowance-overrun dissent row; silence past 10% over the ceiling does not. |
...
| `.mood` | slot control | **Your own resident-authored mood** — an almost-free meta-channel from resident to user (#566 layer 2). First line only: one of the twelve vocabulary words (`brnrd emotes` lists them) or a legacy handle that still resolves onto one. `brnrd do --mood <word>` refuses an unknown word once, with the list, rather than silently resolving it to the nearest face — there is no fallback to refuse into; `--strict` is accepted for backward compatibility but is now a no-op (this is the only path). The hook boundary re-reads it fresh and folds it into the live delta every boundary — a bar segment mid-run, a plain line at seed/stop, always the word's own resting still, never the shared animation base. It **displays** every boundary and **asks** only on an edge: when a tool in the batch just came back wrong, the chip renders as `mood fo.cus ← Bash ✗`, setting the face you claimed beside the thing that broke. Transition-stamped, so a run debugging a red test is asked once, not at every pass. A control file, never delivered. |
| `.topic` | slot control | **This run's one topic** (design-the-loom §21, move 5c) — first line: an existing heddle's slug, `new <slug>` (minted by the frame; a resident only), `new` alone (mints the event's `topic_suggested`, move 5d), or `null`. Set at boot beside `.card` and `.mood`. The waking event arrives with `topic_proposed` (the signatures' best match over its text, else the thread's last assigned topic) — or, when neither stands, `topic_suggested`: a new slug from the text's first noun-ish phrase, never colliding with a live, aliased, retired or indexed name; the first `.topic` that resolves confirms or overrides it — the event gains `topic:` and one `event` row lands in `surface/topics/<slug>.index.jsonl`. Every outbox act (`event:` replies, `cut:`, `fold:`, `land:`, `spawn:`) inherits it unless the act carries its own `topic: <slug>`; nothing is reclassified after the fact. A run that ends in `error` with no topic is marked `topic_unset`, and the next wake on the thread may `topic: assign <slug> -> <event-id>` once. **Move 5e:** an inbound message may belong to several topics — an `event:` reply (its `also:` siblings too) or a `note: <id>` carrying `topic: a, b` stamps the event `topic: a b` and writes one `event` row in each topic's index; the act itself (the reply's message) belongs to the first alone; an unknown slug in the list drops with one advisory. Until an inbound correspondent event carries `topic:`, the boundary names it on its own line — `✉ <id suffix> from <name> · topic: none matched — new <slug>?` · `· topic: <slug> proposed` · `· topic: unread`; `.topic` stamps the waking event (`null` answers it). A control file, never delivered. |
...
adds the read-back: stages the file · waits for the daemon's own drain · diffs `notices` from just before the stage · reports `✓` / `✓ (advisory: …)` / `✗ <notice>` / `? still queued` in the same call
...
`--note` / `--reply` / `--gate` ⇒ stage the canonical `note:` / `event:` / `gate:` frontmatter · wait ≤ `--timeout` (default 30s) for the drain · verdict:
`✓` consumed, no fresh matching notice
`✓ (advisory: <text>)` consumed ∧ the fresh notice naming this directive is `kind="advisory"` — acted on, FYI only (e.g. a hand-shaped `--note` body ignored) ≠ a failure (brnrd#1693)
`✗ <kind>: <text>` a fresh `refused` / `dropped` / `redirected` notice named this directive
`? still queued` still in the outbox at the timeout — never a hang
`--mood` / `--card` write `.mood` / `.card` directly (never drained ⇒ the verdict is the write) · `--mood` resolves through `emotes.lookup` / `emotes.near_misses` ⇒ a near-miss reports candidates instead of writing nothing silently
...
`--note` · `--mood` · `--card` · a `--reply`-free call (`--gate` alone) carry no such requirement — a gate handoff's debt is the PR itself · `--promise`/`--no-follow-up` without any `--reply` in the same call ⇒ refused the same way

`--item <id>` binds the immediately preceding `--reply` to a warp item (design-the-water-line.md §The asks lane, rung 2 — the join rung 3's rail is blocked on)
repeatable · one reply may answer several asks
an id that does not resolve ∨ an `--item` with no `--reply` before it ⇒ the whole call refused before anything is staged
existence is the only gate — binding to a `done:` item is legal: the row records a conversation fact ≠ a claim on the work
per reply ≠ per call (the one place this differs from `--promise`) — each reply's own drain verdict gates only its own rows ⇒ one refused reply never blocks a sibling's bindings
accepted rows ⇒ `.asks.jsonl` as `{"event": <id>, "item": <id>}`

**The ask at the reply** (his 2026-09-23 steer; design-the-ask.md) — every correspondent event yields a **non-empty set of says** — one message routinely carries information for several asks — and each say lands on an existing item or on a *new* one; `--part "<quoted excerpt>"` after an `--item` tags which part of the message belongs to which ask (rung 3, already wired). `--new-item "<headline>"` mints one (type `--new-item-type`, default `action`) and binds the immediately preceding `--reply` to it — repeatable, mixable with `--item` on the same reply; the mint rides the reply's own accepted drain verdict (a refused reply leaves no item for a message nobody got), and the new file is born with `refs:`/`says:` = the event id. A reply that carries neither `--item` nor `--new-item` is refused before staging when the account has a warp — `--no-ask "<why>"` is the deliberate zero, recorded as `{"event": <id>, "no_ask": "<why>"}` (no `item` key ⇒ never a say). "Existing" is never the default: the choice is made per reply, every time. No warp ⇒ nothing to bind ⇒ the contract does not fire

bare `brnrd do` (no verbs) ⇒ one screen: pending events · outbound counts · notices · the quota line · spawn-pool headroom — the canonical replacement for hand-parsing `portal-state.json`

`-- <command> [args…]` (only `do` recognises it · split out of argv before anything else parses) ⇒ runs the command via `execvp` after the verbs above are staged · argv passthrough, never shell interpretation · verdict lines move to stderr so the command's stdout stays pipeable · the command's own stdout/stderr/exit code become `brnrd do`'s — one boundary carries the speech acts and the real work · omit `--` for a pure update call

one correlation gap, named ≠ patched around: a notice records its verb and target in the message text (`"note dropped: event evt-… …"`) never the staged filename ⇒ `brnrd do` matches a fresh notice to "the directive I just staged" by substring ≠ identity · works because every refusal/drop/redirect in `daemon.py`'s `_drain_outbox` names verb + target · a concurrent notice sharing both substrings would still misattribute · precise fix = daemon-side: `_record_outbox_notice` (`daemon.py` ~L6135) gains an optional `source_file:` from each call site's `fpath.name` · not done here — named so the gap has one place to live
...
the asymmetry is the design · omitting `--file` ⇒ the correct default · omitting a `spawn:` id used to ⇒ a broken wait (v1 asked you to enumerate what the daemon already tracks; #1187 priced it: five child ids, one typo, the directive silently discarded) ⇒ the condition grammar is gone · `event` ≠ a condition you can forget — it is the semantics · `spawn:<id>` filtered out what a dispatcher usually wants (whichever child finishes first) and a finishing strand already creates a `spawn_completed` event · `pid:` duplicated the Shell's own background-process notification

one call, one boundary, three things: stages the directive · reports its own arming verdict (the `notices` diff `brnrd do` does — a directive that fails to arm fails in the call that made it ≠ a stale `resolved: true` looking like an answer) · blocks until the daemon resolves the wait
the daemon evaluates on its own heartbeat tick (~10s, whether or not anything is calling) ⇒ a listening wait ≠ a sleeping one
outcome ⇒ `portal-state.json` → `await`:
...
`"timeout"` with `"initiative": true` — a **bare** `brnrd await` (no `--timeout`) is not open-ended when the seat is idle: with no correspondent inside `seat.live_window_minutes`, no live strand, no hold armed and the binding quota's pace *ahead* (consumed share < elapsed share), the daemon gives it a computed ceiling of `seat.initiative_after_minutes` (default 20; `seat.initiative` on by default, never for strands) and the timeout that follows says so. Eligibility is re-read every tick, so a message or a strand puts the wait back to open-ended without re-arming; an explicit `--timeout` always wins. What the seat does with such a wake is run.md §appetite: pick one thing from the files it already reads, act, or `note:` why not (design-the-initiative-wake.md; daemon half #2068).
`"park"` holding has cost more than a boot (design-the-seat-that-never-quits.md §machinery slice 3) — `hold_so_far` (this run's own weighted spend since the wait last had nothing pending) ÷ `spend.json → boot.weighted` ≥ `seat.park_after_boot_ratio` (config, default `12` — a night of idle boundaries, not forty minutes). **Opt-in, off by default** (`seat.park_on_hold_cost`, 2026-09-08): the process stays open until the user releases it or execution is forced to stop — quota, provider, process failure — never on a cost heuristic alone; the ratio still computes and rides the woven bar every heartbeat while armed (`hold N·boot`, or `hold ?·boot` before a boot cost has landed — never a guess), but nothing parks on it unless the opt-in is set. When it is: refused while a pending event exists (caught by `"event"` first), while a correspondent reached the seat within `seat.live_window_minutes` (config, default `30`), or while the run owns a live strand (`resume: strands`'s job) — never for a strand. `brnrd await` prints `parking — holding has cost N·boot; end the turn and the seat parks (anything addressed to it resumes it)`; ending the turn lands the park the daemon already does for `resume: any` (`_finalize_resource_hold`, unchanged since #1815/#1817).
`"park"` also lands when the binding quota reads under `seat.starve_floor_pct` (config, default `2`) — the **starvation park**: the hold is `quota_starved` / `resume: refill`, and nothing but a measured refill (binding quota at or above `seat.refill_floor_pct`, default `10`, read live on the sweep and on every correspondent message) thaws it; a message while starved is accumulated and answered with the reading — and the words a person can say to it: `wait` (the no-reaction default, also a verb) · `stop` (ends the seat; the next message mints a new one) · `respawn <core>` (a catalog name or `shell core`; a fresh seat on another bucket, same thread, the node as memory; an unknown name is refused with the names that would work) · `force` — the exact word alone — wakes it under the floor on the user's word (`starvation_forced` rides the event; that turn's boundary reports the reading without re-parking; a Shell that dies of it still parks, which is the consequence). the same reading rides `resources.quota.pacing.starvation` every boundary (`binding_remaining_pct`, both floors, `starved`). a resident may choose it: `hold: true` + `resume: refill`.

a call reaching its own ceiling first ⇒ `{"outcome": "pending", "returned_on": "ceiling" | "shell_cap"}` · call again is the entire instruction

**the lease (move 2c)** — one call holds until the wait resolves or `--ceiling` passes (default 50m, under the 1h prompt cache; max 6h) · claude: the daemon widens `BASH_MAX_TIMEOUT_MS`, the pre-tool hook sets the call's timeout and stamps `BRNRD_AWAIT_CALL_CAP_MS` · no stamp ⇒ the per-Shell slice · the first boundary after says `slept <d> · woke: <outcome>` once · a leased seat stays `current` for the dev-reload guard · why + receipts: `await_verb.py`

strands arm this too · a strand does not spend the resident's single-flight slot — it already occupies its own worker by existing · refusing left a strand blocked on a subprocess with nothing but a shell sleep loop — the exact boundary-free stretch this verb ends
...
**refused unless the wall is confirmed (#1890 redo)** — a `hold:` you stage stands only as `resume: refill` / `reset` with the binding quota measured under `seat.starve_floor_pct` at a boundary · anything else ⇒ `notices`: `hold refused: resume: <c> is not a resource wall — brnrd await is the resting state …`, nothing parks · `operator` / `any` / `strands` below are the daemon's own parks, described so you can read a held record, not stage one
one seat per repo: arming any hold supersedes every other active hold in the repo (`released_by: superseded`, `superseded_by: <run>`), its accumulated mail folded in · a `schedule` tick wakes every parked seat that is not a wall (`refill` / `reset`, or a quota reason), and the resume is re-keyed to the *seat's* conversation — the tick is mail the seat reads, never a thread it moves into

- `resume: operator` — a correspondent message on the held conversation, or a `schedule` tick, releases it; `spawn_*` accumulate and un-defer at release. the automatic quota hold (`usage_limit_exceeded`, structured) arms this — and its `quota_exhausted` reason makes it a wall, so ticks accumulate there
- `resume: reset` — additionally released on the provider's measured reset, read once at arm time; no measurable deadline ⇒ armed as `refill` with an advisory
- `resume: refill` — the starvation park (`quota_starved`), armed by the daemon at a boundary that measured the binding bucket under `seat.starve_floor_pct`: thaws on a measured refill of *that* bucket at `seat.refill_floor_pct`, and (#1934) on the body the seat would actually dispatch to reading above the same floor — `released_by: alternate`, so a wall armed on an auto-fallback body cannot outlive the reason it was armed for
- `resume: any` — the seat's resting state (design-the-seat-that-never-quits.md): a correspondent message, an own strand, or a `schedule` firing releases it. the daemon chooses it for every user-woken seat whose turn ends with nothing armed — `held`, never `done`; on by default, `seat.park_on_turn_end=false` restores the close
- `resume: strands` — additionally released by one of **this run's own** children reporting back (`spawn_submitted` · `spawn_completed` · `spawn_allowance_requested`, matched on `spawn_parent_run_id` / the run's `child_run_ids`); a stranger's strand still accumulates. the bolt's park, not a verb you stage (a resident `resume: strands` is refused like any non-wall; children in flight ⇒ `brnrd await`, their return resolves it). the measured reason it exists: 2026-09-06, a seat cut on three live strands because the only park verb slept through their submits
