...
**The ask at the reply** (his 2026-09-23 steer; design-the-ask.md) — every correspondent event yields at least one ask, and each is either a *say* on an existing item or a *new* item; several distinct asks in one event are several bindings. `--new-item "<headline>"` mints one (type `--new-item-type`, default `action`) and binds the immediately preceding `--reply` to it — repeatable, mixable with `--item` on the same reply; the mint rides the reply's own accepted drain verdict (a refused reply leaves no item for a message nobody got), and the new file is born with `refs:`/`says:` = the event id. A reply that carries neither `--item` nor `--new-item` is refused before staging when the account has a warp — `--no-ask "<why>"` is the deliberate zero, recorded as `{"event": <id>, "no_ask": "<why>"}` (no `item` key ⇒ never a say). "Existing" is never the default: the choice is made per reply, every time. No warp ⇒ nothing to bind ⇒ the contract does not fire
