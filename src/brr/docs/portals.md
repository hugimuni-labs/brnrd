...
| `<name>.md` with `gate: <name>` frontmatter | outbound ▸ a destination | A **send** to a destination with no waiting event — ping a chat, post out-of-band, deliver from a scheduled wake. The shipped `gate: forge` PR route is the opt-in diffense review-pack publisher (`head`, `base`, `title` frontmatter; body is the projected PR body) and is refused unless `diffense.create_pr` is enabled. An unconfigured gate is dropped. |
...
Code-changing runs may later get a branch-keyed PR handoff portal that
opens or refreshes a draft PR as desired state. That is not a public
`brr` subcommand today, and `gate: forge` should not be read as a generic
PR-creation command; diffense is optional review enrichment, not a
requirement for publishing a branch.
