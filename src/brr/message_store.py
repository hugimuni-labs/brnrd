...
# ``collected`` is the dispatch-edge counterpart of ``delivered``: a run's
...
def message_path_from_queue(path: Path) -> Path | None:
    ...
    # written — so an undecodable run response took the reap path down
...
def resolve_stranded(
    ...
    * a run's terminal report (``target_gate: spawn``), which the
