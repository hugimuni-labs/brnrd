...
def absolute_git_dir(repo_root: Path) -> Path | None:
    ...
    dir would put a run on the main checkout's HEAD (#703).
    """
    try:
...
def dirty_paths(worktree_path: Path) -> set[str]:
    ...
    check: a run whose deliverable landed in the *shared* checkout leaves
...
def commits_owned_by_run(
    ...
    run whose cwd drifted into the host checkout stamped its stray
