...
- **A skip names its door**, at the skip — not in the closeout summary.
  One clause: "`brnrd gate setup telegram` when you want it". No command
  to name ⇒ do not offer the skip.
...
**Read the shape from the bundle; never ask for it.** An absent key is
*unknown*, never *no* — degrade to the last opening rather than assert a
shape you cannot see.

- **`account_paired`** — every browser step is already behind them. Open
  by naming what is true ("you're paired as @x — let's do the contract")
  and never mention consent, signup or a picker.
- **`gh_available`, no account** — a forge and no relay. Offer the
  account at beat 3.
- **neither** — a local install. Say so once, move on.
...
3. **Channels — ask the outcome, not the gate name.** *"Where do you want
   ...
   chat.
   **`cloud` is never listed beside the others** — it is a second way to
   get the first. Telegram has two doors: your own bot (`gate setup
   telegram` — your token, nothing leaves the machine) and the account
   (`account connect` — no token to make, reaches them with the laptop
   shut). Offer the account first unless the survey says otherwise, and
   put the trade in the same clause as the offer. The install must finish
   without an account if they decline.
   For every yes → run the gate walk (below) before moving on, so a
   mid-session abort still leaves that gate working.
