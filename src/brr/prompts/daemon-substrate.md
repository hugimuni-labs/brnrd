...
- **frontmatter routes the file:**
  ...
  | `await: true` | hold this run until the daemon has something — any pending event resolves it · `brnrd await` = the verb · the process stays alive: **free while blocked: one call holds a lease until something resolves it** (claude: up to 6h — the hook sets the call's timeout; codex: `pending` at its per-call cap, call again) — no boundary while it holds; the first one after says `slept · woke:` |
