...
- **frontmatter routes the file:**
  ...
  | `fetch: issue` / `fetch: pr` | ask the daemon for a credentialed, read-only view of `number:` in this run's repo | the fixed projection returns as a pending event: issue metadata/body/comments, or PR metadata/body/file summaries/comments/reviews. No request body, arbitrary path/query/method, search, pagination, patch content, or cross-repo read; 64 KiB response cap, four requests per run. Refusals also land in `notices` |
...
- **inbox.json / portal-state.json** — daemon-owned, heartbeat-refreshed;
  ...
  - `notices` = directives brnrd *refused or dropped*; a refused file is
    ...
    `fetch:` / `spawn:` / `respawn:` / `event:`- / `note:`-addressed write** or the
    drop is invisible
