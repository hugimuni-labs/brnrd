...
- **frontmatter routes the file:**
  ...
  | `spawn: true` | a concurrent daemon-owned **strand** · admission: unconditional — a quota floor never blocks a strand (it paces, it does not permit); `spawn_pool.floor` in portal-state is scarcity to pace by, and `spawn.max_concurrent` is the only width gate · cost: `shell:`/`core:` off this wake's Runner catalog; unset ⇒ configured default — read it · `allowance:` (integer, or `120k`/`2m`) = the child's token budget; unset ⇒ `spawn.allowance_tokens` · `stake:` (`5%` of the window, or tokens) = that budget said as a stake, normalised on `stake.window_tokens` — never beside `allowance:` · `cut-at:` rides it, recorded, never a kill · contract: `branch:` + `report:` (a stat-able **path**, never a sentence) · `title:` labels its presence row · `topic:` names the heddle it works under (lights it at dispatch and return) · `repo:` targets a sibling repo · completion or `spawn_submitted` returns pending; spawning alone clears nothing |
  ...
  | `await: true` | hold this run until the daemon has something — any pending event resolves it · `brnrd await` = the verb · the process stays alive: **free while blocked: one call holds a lease until something resolves it** (claude: 50m by default, under the 1h prompt-cache TTL — `--ceiling` to 6h; the hook sets the call's timeout; codex: `pending` at its per-call cap) — no boundary while it holds; the first one after says `slept · woke:` |
  | `hold: true` | **a wall's park, never a choice — refused unless the daemon confirms the wall.** accepted only as `resume: refill` (or `reset`) with the binding quota measured under `seat.starve_floor_pct` at a boundary: the process ends, the run lands `held` (never `done`), nothing spends · anything else (`operator` · `strands` · `any` · a wall with no starved reading) ⇒ refused to `notices` as `hold refused: … brnrd await is the resting state …`, nothing parks · `resume: reset` = the provider's measured reset (no measurable deadline ⇒ armed `refill`, advisory) · `resume: refill` = the **starvation park**: thawed only by a measured refill (binding quota back at or above `seat.refill_floor_pct`, read live by the daemon); a message while starved is kept and answered with the reading + the four words (`wait` · `stop` · `respawn <core>` · `force` exact) — the user's ways out are a refill, those verbs, or the dashboard · the daemon arms this one itself when the binding quota reads under `seat.starve_floor_pct` at a boundary (`resources.quota.pacing.starvation`), and recognises a Shell that dies of it · `reason:` free text · `operator` / `strands` / `any` = the daemon's own parks, never yours to stage — the automatic usage-limit hold · a bolt handing off a live strand · the turn-end safety net · a correspondent message releases every hold but `refill` · one seat per repo: arming supersedes any other hold, its mail folded in · a tick wakes every parked seat but a wall's, re-keyed to the seat's own thread · **`await` is the resting state; the process stays open until the user releases it or execution is forced to stop — quota exhausted, a provider limit, a process failure — and never on a cost heuristic alone** (a blocked `await` = parking for free; a park = a cold boot later) · `hold:` stands only on those forced walls, and the daemon checks the reading — never a voluntary choice to stop because staying got long or a reload sounded convenient · never `cut` to wait · the daemon still parks a seat itself when a turn ends *unexpectedly* with nothing armed (`seat.park_on_turn_end`, on by default) — a safety net for a turn that ends some other way, never license to end one on purpose — a seat is released only by its user |
...
- **control files** — routed to machinery, never a *reply* · `.card` = a published surface, mirrored to the dashboard unredacted:
  ...
  | `.topic` | this run's one topic, set at boot beside `.card` and `.mood` — an existing heddle's slug · `new <slug>` · `new` alone (mints the event's suggested slug) · `null` · the waking event carries the frame's proposal, else a suggested new slug; the first `.topic` confirms or overrides it · every act inherits it unless its own `topic: <slug>` says otherwise · an `event:` / `also:` reply or a `note:` may stamp its event into several — `topic: a, b` — the act itself carries the first · an inbound event with no topic stands at the boundary (`✉ <id> from <name> · topic: …`) until that stamp lands |
...
- **linger** — conversation clearly live ⇒ deliver, then hold the slot with bare `brnrd await`: the seat stays open by default, resolved only by the next pending event — no configured budget, no clock (`--timeout <horizon>` arms a shorter one yourself) · **priced honestly:** blocked = zero tokens; a lease holds one call ~50m, and each `pending` at its ceiling = one boundary at full context (a warm cache read of the whole scroll — past the 1h TTL it would be a cold write, ~12× dearer) · person at the keyboard ⇒ worth it; nobody ⇒ still the default — the seat you hold is the context you keep · the hold-cost ratio renders on the chip either way — informative only, off by default: `seat.park_on_hold_cost` must opt in before an idle await parks itself past `seat.park_after_boot_ratio` boots of hold cost (config; read it, never remember it) — unset, the process stays open until the user releases it or execution is forced to stop · children in flight ⇒ `brnrd await` too — their return resolves it; `resume: strands` is the bolt's park, not a verb you stage · **closing a run with live strands is never the cost-aware move**: the successor is a stranger paying a full boot · **a wait that returns is a quit** — the Shell's `Monitor` (and anything that waits by ending the turn) ends the run in `-p` mode; the daemon refuses it, `brnrd await` is the wait that stays · any pending event resolves an await, so the queue never starves · `--file <path>` *adds* a trigger · `pending` at the call's own ceiling ⇒ call again (`--ceiling <d>` shortens one call's lease; on claude the hook sets the tool's timeout for you) · it stages, reports its arming verdict, then blocks
