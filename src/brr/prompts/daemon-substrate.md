...
- **frontmatter routes the file:**
  ...
  | `to: <run-or-event-id>` | mid-flight steer to a child this run dispatched | lands as an event only that strand's `inbox.json` / portal-state shows; the child folds it in — not a new contract, not for `event:`-addressing; unconsumed ⇒ dies with the child. Strands are thread-isolated — steer through this verb, never prose in the thread |
