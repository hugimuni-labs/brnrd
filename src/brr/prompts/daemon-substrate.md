...
- **linger** — conversation clearly live ⇒ deliver, then hold the slot with `brnrd await --timeout <horizon>` (~30m past last delivery; longer vigils = scheduled wakes). **the wait is nearly free** — a blocked await spends zero tokens; closing early trades a free hold for a cold restart. waiting-on-Stop = the cheaper default. `brnrd await`, bare, is the whole shape: any pending event resolves it, so the queue never starves · `--timeout` defaults to the run's remaining budget · `--file <path>` *adds* a trigger · `pending` at the call's own ceiling ⇒ call again (mind the Shell's per-call cap — pass the tool's own timeout too). it stages, reports its arming verdict, then blocks; `.keepalive` is extended for you.
