...
- **control files** — routed to machinery, never a *reply*. `.card` = a published surface, mirrored to the dashboard unredacted:
  ...
  | `.linger-opt-out` | skip the live-chat linger; first line = the reason. moot once the seat parks on turn end — leaving is the user's act, never yours |
...
- **linger** — conversation clearly live ⇒ deliver, then hold the slot with bare `brnrd await`: the seat stays open by default, resolved only by the next pending event — no configured budget, no clock (`--timeout <horizon>` arms a shorter one yourself). **priced honestly:** blocked = zero tokens; every `pending` at the Shell's per-call cap = one boundary at full context (a cache read of the whole scroll — on a 160 KB wake that is visible on the session gauge). a person at the keyboard ⇒ worth it, every time; nobody at the keyboard ⇒ still the default — the seat you hold is the context you keep, and a resume is a boot. an idle await parks itself only past `seat.park_after_boot_ratio` boots of hold cost (config; read it, never remember it). `hold: true` + `resume: strands` is the exception for a run that *knows* it is leaving for hours with children in flight. **closing a run with live strands is never the cost-aware move**: the successor is a stranger paying a full boot. **a wait that returns is a quit** — the Shell's `Monitor` (and anything that waits by ending the turn) ends the run in `-p` mode; the daemon refuses it, `brnrd await` is the wait that stays. any pending event resolves an await, so the queue never starves · `--file <path>` *adds* a trigger · `pending` at the call's own ceiling ⇒ call again (mind the Shell's per-call cap — pass the tool's own timeout too). it stages, reports its arming verdict, then blocks.
