...
governance, this page — every default wake) · the bounded wake (task + files
+ result contract, nothing standing) — a **strand** when `spawn:` throws one
as a concurrent sibling, the same shape when a handoff walks into it. Opt-in
on a handoff: `worker: true` beside `respawn: true`; left off, a respawn is a
