...
def build_boot_score(
    ...
    *,
    ...
    # Built before the score because the kernel renders the walk's reach.
...
def _prior_run_node(repo_root: Path) -> tuple[Path, Path] | None:
    ...
    **A strand's own node is excluded too.** The rule being enforced is "a
    parked seat resuming gets *its own* node, not an older one" — found while
    verifying #1827 (context-floor rebirth), true independently of it, and
    fixed here on its own. The live
